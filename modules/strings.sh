...
analyze_strings() {
    ...
    LC_ALL=C run_cmd strings "$f" | tr -d '\0' > "$tmp_str"

    while read l; do
        emit_keyword "Keyword: $l"
    done < <(grep -iE "$kw" "$tmp_str")

    while read b64; do
        local d=$(echo "$b64" | base64 -d 2>/dev/null | tr -d '\0')
        if [ -n "$d" ] && echo "$d" | grep -qiE "$kw" 2>/dev/null; then
            emit_finding "base64_string" "Base64: $d"
        fi
